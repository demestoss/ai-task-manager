import type { Task, TaskCategory, TaskDescriptionType, TaskPriority } from '@repo/domain/task';

const priorities: TaskPriority[] = ['high', 'low', 'medium', 'useless'];
const categories: TaskCategory[] = ['coding', 'hobby', 'home', 'work', 'other', 'reading'];

export function getTaskPrompt(name: string, descriptionType: TaskDescriptionType) {
  switch (descriptionType) {
    case 'long':
      return getTaskPromptWithDescription(name);
    case 'none':
      return getTaskPromptWithoutDescription(name);
    case 'short':
      return getTaskPromptShortDescription(name);
  }
}

function getTaskPromptWithDescription(name: string) {
  return `
...
function getTaskPromptWithoutDescription(name: string) {
  return `
    I'm writing todo item for myself for the future
    ...
    The name should be also generated based on the todo item title

    The dueDate should be generated if it's possible to estimate the due date based on the todo item title

    The output from you should be the json with fields: name, priority, category, dueDate
    Nothing else should be provided except json
...
function getTaskPromptShortDescription(name: string) {
  return `
    I'm writing todo item for myself for the future
    ...
    The description and name should be also generated based on the todo item title
    The description should be short and include only the most important information
...
export function remakeTaskPrompt(task: Task, prompt: string) {
  return `
     I'm writing todo item for myself for the future
    ...
    This the available priorities: ${priorities.join(', ')}
    
    This is existing task:
    ${JSON.stringify(task, null, 2)}
    
    Please update the task based on the prompt:
    ${prompt}
